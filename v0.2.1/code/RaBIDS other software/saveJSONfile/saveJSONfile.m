function saveJSONfile_cp(data, jsonFileName)
...
% Change log:
% - Christian Paret: added exemption to recognize character cell arrays
...
function writeSingleElement(fid, currentField,currentElementValue,tabs)
        ...
        if length(currentElementValue) > 1 && ~ischar(currentElementValue) && ~iscell(currentElementValue)
            fprintf(fid,' "%s" : [\n%s',currentField,tabs);
            ...
            fprintf(fid,'\n%s]\n%s',tabs,tabs);
        elseif iscell(currentElementValue) % CP: if this is a character cell array, write character strings to item
            fprintf(fid,'"%s" : [',currentField);
            for m = 1:length(currentElementValue)-1
                fprintf(fid,'"%s",\n%s%s%s%s%s',currentElementValue{m},tabs,tabs,tabs,tabs,tabs);
            end
            if isempty(m)
                m=1;
            else
