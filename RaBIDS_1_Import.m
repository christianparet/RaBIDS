    ...
    for j = 1:length(ses_id)
        ...
        if ~isfolder(subject_dir) || contains(overwrite,'y')
            ...
            if ~contains(scanprotocol.name,'no scan found')
                uni_vols=unique(scanprotocol.vols); 
                dup_vols=uni_vols(hist(scanprotocol.vols,uni_vols)>1); %find volumes with identical numbers
                uni_names=categorical(scanprotocol.name); 
                dup_names=categories(uni_names);
                dup_names=dup_names(countcats(uni_names)>1); %find volumes with identical names
                for o = 1:length(scanprotocol.name) 
                    dupvol=dup_vols(scanprotocol.vols(o)==dup_vols);
                    dupname=dup_names(ismember(scanprotocol.name(o),dup_names));
                    vol_ident=(ismember(scanprotocol.vols,dupvol))+(ismember(scanprotocol.name,dupname));
                    if ~vol_ident(o) 
                        Torder(o)=0; %first set zeros to non-repeating scans
                    elseif ~isempty(Torder)
                        [duprow]=find(strcmp(scanprotocol.name,dupname)); %gets rows with the same names
                        last_duprows=duprow-length(Torder);
                        lastduprow=max(last_duprows(last_duprows<=0)); %identify the distance of the last row with the same number of volumes
                        if ~isempty(lastduprow)
                            Torder(o)=Torder(o-1+lastduprow)+1;
                        else
                            Torder(o)=1;
                        end
                    end
            ...
            for k = 1:length(tasks)
                ...
                for l = 1:nr
                    if strcmp(scanprotocol.name{l},metadata)
                        if (scanprotocol.vols(l)>=n_min && scanprotocol.vols(l)<=n_max && scanprotocol.order(l)==order) || (scanprotocol.vols(l)==n_min && isnan(n_max)
