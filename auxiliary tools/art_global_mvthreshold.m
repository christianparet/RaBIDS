function art_global_mvthreshold(Images,RealignmentFile,HeadMaskType, RepairType, mv_thresh)
% Note: this is an adapted version of art_global.m with the option to
% define the movement threshold for artifact detection
% by Christian Paret, ZI-Mannheim, 2021
...
if nargin > 0
    ...
    repair1_flag = 0;   % Only repair scan 1 when necessary % CP: commented in, 2021/04/13
%     repair1_flag = 1;   % Force repair scan 1  (Reisslab) % CP: commented out, 2021/04/13
    GoRepair = 1;       % Automatic Repair
    if nargin == 5      % To stay backward compatible with v2.1 % CP: changed to nargin == 5, 2021/04/13
        if RepairType == 1
            GoRepair = 1;
        elseif RepairType == 2
            mv_thresh = 0.5;
...
elseif realignfile == 0
	...
	legend('x mvmt', 'y mvmt', 'z mvmt','Location','Best');
	h = gca;
	set(h,'Ygrid','on');
end 
